module Lab42
  module Meta
    module Hash extend self
      def hash_merge_rec orig, keys, valblock, limits
        return unlimited_hash_merge_rec orig, keys, valblock unless limits
        limits = ::Hash[ *keys.zip( [limits] * keys.size ).flatten ] unless ::Hash === limits
        limited_hash_merge_rec orig, keys, valblock, limits
      end

      private
      def limited_hash_merge_rec orig, keys, valblock, limits
      ...
      def recur_or_value keys, v, valblock, limits
        if ::Hash === v
          limited_hash_merge_rec v, keys, valblock, limits
        else
          v
        end
      end

      def unlimited_hash_merge_rec orig, keys, valblock
        orig.inject ::Hash.new do | h, (k, v) |
      ...
      def unlimited_value keys, k, v, valblock
        if keys.include? k
          ...
          unlimited_hash_merge_rec v, keys, valblock
        else
          v
        end
      end
