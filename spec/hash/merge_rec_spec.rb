...
RSpec.describe Hash do
  ...
  context "#merge_rec" do 
    context "nilpotent if keys are not present" do 
      it "as in empty hashes" do
        expect( {}.merge_rec( :a, &Integer.fm.succ ) ).to be_empty
      end
      it "as in keys are not there" do
        expect( {c: 42}.merge_rec( :a, :b, &Integer.fm.succ ) ).to eq c: 42
      end
    ...
    context "behaves like merge" do 
      ...
      context "with one key it changes that value" do 
        it "in the result" do
          expect( receiver.merge_rec( :a, &Integer.fm.succ ) ).to eq a: 2, b: 2
        end
        it "but not in the receiver" do
          receiver.merge_rec( :a, &Integer.fm.succ )
          expect( receiver ).to eq a: 1, b: 2
        end
      end # context "with one key"
      context "with two keys it changes the values" do 
        it "in the result" do
          expect( receiver.merge_rec( :a, :b, &Integer.fm.succ ) ).to eq a: 2, b: 3
        end
        it "but not in the receiver" do
          receiver.merge_rec( :a, :b, &Integer.fm.succ )
          expect( receiver ).to eq a: 1, b: 2
        end
    ...
    context "but also works in subhashes" do 
      ...
      it "changes all values of a given key" do
        expect( complex.merge_rec(:a, &:succ) ).to eq a: 2, x:{a: 3, b: 10, x: {a: 4, b: 20} }
      end
      it "changes all values of all given keys" do
        expect( complex.merge_rec(:a, :b, &:succ) ).to eq a: 2, x:{a: 3, b: 11, x: {a: 4, b: 21} }
      end
      it "changes only a certain number of values (for one key)" do
        expect( complex.merge_rec :a, limit: 2, &:succ ).to eq a: 2, x:{a: 3, b: 10, x: {a: 3, b: 20} }
      end
      it "changes only a certain number of values (for some keys)" do
        expect( complex.merge_rec(:a, :b, limit: 1, &:succ) ).to eq a: 2, x:{a: 2, b: 11, x: {a: 3, b: 20} }
      end
      it "can have different limits per key" do
        expect( complex.merge_rec(:a, :b, limits: {a: 2, b: 1}, &:succ ) ).to eq a: 2, x:{ a: 3, b: 11, x: {a: 3, b: 20}}
      end

      it "and all keys do not have to posess defined limits" do
        a = {a: 1, b: 10, x: {a: 2, x:{a: 3, b: 20}}} 
        expect( a.merge_rec( :a, :b, limits: {b: 1}, &:pred )).to eq a: 0, b: 9, x: {a: 1, x:{a: 2, b: 20}}
      end
    ...
    context "replace order is outside inside" do 
      ...
      it "therefore cuts a subhash if it has a replacement key" do
        expect( subkey.merge_rec(:a, :b, &repl) ).to eq a: {a: 100}, x: { a: {a: 100}}
      end
      it "unless limits avoid this, in which case we recurse anyway" do
        expect( subkey.merge_rec(:a, :b, limit: 1, &repl) ).to eq a: {a: 100}, x: { a: { a: 2, b: {a: 100}}}
                                                                                     
      end
    end # context "replace order is outside inside"
  end # context "#merge_rec"
end
